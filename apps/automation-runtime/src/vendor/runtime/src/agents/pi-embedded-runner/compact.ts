...
import { resolveRuntimeAgentDir } from "../agent-paths.js";
...
import { resolveRuntimeDocsPath } from "../docs-path.js";
import { getApiKeyForModel, resolveModelAuthMode } from "../model-auth.js";
import { supportsModelTools } from "../model-tool-support.js";
import { ensureRuntimeModelsJson } from "../models-config.js";
...
import { createRuntimeCodingTools } from "../pi-tools.js";
...
export async function compactEmbeddedPiSessionDirect(
  ...
  const agentDir = params.agentDir ?? resolveRuntimeAgentDir();
  await ensureRuntimeModelsJson(params.config, agentDir);
  ...
  try {
    ...
    const toolsRaw = createRuntimeCodingTools({
      exec: {
    ...
    const promptMode =
      isSubagentSessionKey(params.sessionKey) || isCronSessionKey(params.sessionKey)
        ? "minimal"
        : "full";
    const docsPath = await resolveRuntimeDocsPath({
      workspaceDir: effectiveWorkspace,
    ...
    try {
      ...
      if (model.api === "ollama") {
        throw new Error(
          "Ollama models are not supported by automation-runtime compaction. Configure a Gemini model in the UI.",
        );
      }
