...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via RUNTIME_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `RUNTIME_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${RUNTIME_PREPEND_PATH}:$PATH"; unset RUNTIME_PREPEND_PATH; '
    : "";
