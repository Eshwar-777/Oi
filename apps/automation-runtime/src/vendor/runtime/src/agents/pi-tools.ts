...
function applyModelProviderToolPolicy(
  ...
  // xAI/Grok providers expose a native web_search tool; sending Runtime's
...
function resolveExecConfig(params: { cfg?: RuntimeConfig; agentId?: string }) {
...
export function createRuntimeCodingTools(options?: {
  agentId?: string;
  ...
  /** If true, restrict Runtime tools to browser-only automation. */
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createRuntimeReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
