...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.RUNTIME_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".runtime", `workspace-${profile}`);
  }
  return path.join(home, ".runtime", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".runtime";
