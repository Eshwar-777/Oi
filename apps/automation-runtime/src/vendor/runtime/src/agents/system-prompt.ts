...
function buildMessagingSection(params: {
  ...
  return [
    ...
    "- Never use exec/curl for provider messaging; Runtime handles all routing internally.",
...
function buildDocsSection(params: { docsPath?: string; isMinimal: boolean; readToolName: string }) {
  ...
  return [
    "## Documentation",
    `Runtime docs: ${docsPath}`,
    "Mirror: https://docs.runtime.ai",
    "Source: https://github.com/runtime/runtime",
    "Community: https://discord.com/invite/clawd",
    "Find new skills: https://clawhub.com",
    "For Runtime behavior, commands, config, or architecture: consult local docs first.",
    "When diagnosing issues, run `runtime status` yourself when possible; only ask the user if you lack access (e.g., sandboxed).",
    "",
  ];
}
...
export function buildAgentSystemPrompt(params: {
  ...
  const coreToolSummaries: Record<string, string> = {
    ...
    gateway: "Restart, apply config, or run updates on the running Runtime process",
    agents_list: acpSpawnRuntimeEnabled
      ? 'List Runtime agent ids allowed for sessions_spawn when runtime="subagent" (not ACP harness ids)'
      : "List Runtime agent ids allowed for sessions_spawn",
  ...
  if (promptMode === "none") {
    return "You are a personal assistant running inside Runtime.";
  }

  const lines = [
    "You are a personal assistant running inside Runtime.",
    "",
    "## Tooling",
    ...
    toolLines.length > 0
      ...
      : [
          ...
          "- browser: control Runtime's dedicated browser",
          "- canvas: present/eval/snapshot the Canvas",
          "- nodes: list/describe/notify/camera/screen on paired nodes",
          "- cron: manage cron jobs and wake events (use for reminders; when scheduling a reminder, write the systemEvent text as something that will read like a reminder when it fires, and mention that it is a reminder depending on the time gap between setting and firing; include recent context in reminder text if appropriate)",
    ...
    "",
    ...safetySection,
    "## Runtime CLI Quick Reference",
    "Runtime is controlled via subcommands. Do not invent commands.",
    "To manage the Gateway daemon service (start/stop/restart):",
    "- runtime gateway status",
    "- runtime gateway start",
    "- runtime gateway stop",
    "- runtime gateway restart",
    "If unsure, ask the user to run `runtime help` (or `runtime gateway --help`) and paste the output.",
    "",
    ...skillsSection,
    ...memorySection,
    // Skip self-update for subagent/none modes
    hasGateway && !isMinimal ? "## Runtime Self-Update" : "",
    hasGateway && !isMinimal
      ? [
          ...
          "After restart, Runtime pings the last active session automatically.",
        ].join("\n")
      : "",
    hasGateway && !isMinimal ? "" : "",
    "",
    ...
    "These user-editable files are loaded by Runtime and included below in Project Context.",
    "",
  ...
  if (!isMinimal) {
    lines.push(
      ...
      "HEARTBEAT_OK",
      'Runtime treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).',
