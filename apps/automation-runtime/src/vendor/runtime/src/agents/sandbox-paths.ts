...
import { resolvePreferredRuntimeTmpDir } from "../infra/tmp-runtime-dir.js";
...
async function resolveAllowedTmpMediaPath(params: {
  ...
  const runtimeTmpDir = path.resolve(resolvePreferredRuntimeTmpDir());
  if (!isPathInside(runtimeTmpDir, resolved)) {
    return undefined;
  }
  await assertNoTmpAliasEscape({ filePath: resolved, tmpRoot: runtimeTmpDir });
  return resolved;
}
