...
import { planRuntimeModelsJson } from "./models-config.plan.js";
...
function resolveModelsConfigInput(config?: RuntimeConfig): RuntimeConfig {
...
export async function ensureRuntimeModelsJson(
  config?: RuntimeConfig,
  agentDirOverride?: string,
): Promise<{ agentDir: string; wrote: boolean }> {
  const cfg = resolveModelsConfigInput(config);
  const agentDir = agentDirOverride?.trim() ? agentDirOverride.trim() : resolveRuntimeAgentDir();
  ...
  return await withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planRuntimeModelsJson({
      cfg,
      agentDir,
      env,
