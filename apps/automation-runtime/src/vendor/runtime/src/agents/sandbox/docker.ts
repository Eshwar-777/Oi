...
import { markRuntimeExecEnv } from "../../infra/runtime-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "runtime.sandbox=1");
  args.push("--label", `runtime.sessionKey=${params.scopeKey}`);
  args.push("--label", `runtime.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `runtime.configHash=${params.configHash}`);
  }
  ...
  const envSanitization = sanitizeEnvVars(markRuntimeExecEnv(params.cfg.env ?? {}));
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "runtime.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`runtime sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`runtime sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("runtime sandbox recreate --all");
}
