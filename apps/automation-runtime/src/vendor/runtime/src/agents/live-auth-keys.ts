...
const GOOGLE_LIVE_SINGLE_KEY = "RUNTIME_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "RUNTIME_LIVE_ANTHROPIC_KEY",
    listVar: "RUNTIME_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "RUNTIME_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `RUNTIME_LIVE_${base}_KEY`;
