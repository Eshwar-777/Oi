import { type RuntimeConfig, loadConfig } from "../config/config.js";
...
function readConfiguredOptInProviderModels(config: RuntimeConfig): ModelCatalogEntry[] {
...
export async function loadModelCatalog(params?: {
  ...
  modelCatalogPromise = (async () => {
    ...
    try {
      ...
      await ensureRuntimeModelsJson(cfg);
      // IMPORTANT: keep the dynamic import *inside* the try/catch.
      // If this fails once (e.g. during a pnpm install that temporarily swaps node_modules),
      // we must not poison the cache with a rejected promise (otherwise all channel handlers
      // will keep failing until restart).
      const piSdk = await importPiSdk();
      const agentDir = resolveRuntimeAgentDir();
