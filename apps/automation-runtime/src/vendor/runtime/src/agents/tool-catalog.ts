...
function buildCoreToolGroupMap() {
  ...
  const RuntimeTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInRuntimeGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:runtime": RuntimeTools,
