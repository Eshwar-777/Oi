...
function redactSessionsSpawnAttachmentsArgs(value: unknown): unknown {
  ...
  const next = raw.map((item) => {
    ...
    return { ...rest, content: "__RUNTIME_REDACTED__" };
  });
...
function makeMissingToolResult(params: {
  ...
  return {
    ...
    content: [
      {
        type: "text",
        text: "[runtime] missing tool result in session history; inserted synthetic error result for transcript repair.",
      },
    ],
    isError: true,
    timestamp: Date.now(),
...
export function repairToolUseResultPairing(messages: AgentMessage[]): ToolUseRepairReport {
  ...
  for (let i = 0; i < messages.length; i += 1) {
    ...
    // See: https://github.com/runtime/runtime/issues/4597
