 ...
 * A common mistake — caused by the similarity with the `runtime.json`
...
function normalizeRawCredentialEntry(raw: Record<string, unknown>): Partial<AuthProfileCredential> {
  ...
  // mode → type alias (runtime.json uses "mode"; auth-profiles.json uses "type")
...
function loadAuthProfileStoreForAgent(
  ...
  const forceReadOnly = process.env.RUNTIME_AUTH_STORE_READONLY === "1";
