...
function formatBrowserCliCommand(command: string): string {
  const profile = process.env.RUNTIME_PROFILE?.trim();
  if (!profile) {
    return command;
  }
  if (!/^(?:pnpm|npm|bunx|npx)\s+runtime\b|^runtime\b/.test(command)) {
    return command;
  }
  if (/(?:^|\s)--profile(?:\s|=|$)|(?:^|\s)--dev(?:\s|$)/.test(command)) {
    return command;
  }
  return command.replace(/^(?:pnpm|npm|bunx|npx)\s+runtime\b|^runtime\b/, (match) => {
...
async function refreshBrowserQwenPortalCredentials(
  ...
  if (!response.ok) {
    ...
    if (response.status === 400) {
      throw new Error(
        `Qwen OAuth refresh token expired or invalid. Re-authenticate with \`${formatBrowserCliCommand("runtime models auth login --provider qwen-portal")}\`.`,
      );
    }
...
type SecretDefaults = NonNullable<RuntimeConfig["secrets"]>["defaults"];
...
async function resolveProfileSecretString(params: {
  ...
  configForRefResolution: RuntimeConfig;
