...
function resolveAttachmentLimits(config: RuntimeConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".runtime", "attachments");
  const relDir = path.posix.join(".runtime", "attachments", attachmentId);
