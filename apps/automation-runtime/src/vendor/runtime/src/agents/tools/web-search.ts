...
type WebSearchConfig = NonNullable<RuntimeConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: RuntimeConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  if (provider === "brave") {
    return {
      ...
      message: `web_search (brave) needs a Brave Search API key. Run \`${formatCliCommand("runtime configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
      docs: "https://docs.runtime.ai/tools/web",
    };
  }
  if (provider === "gemini") {
    return {
  ...
  return withTrustedWebSearchEndpoint(
    {
      ...
      init: {
        ...
        headers: {
          ...
          "HTTP-Referer": "https://runtime.ai",
          "X-Title": "Runtime Web Search",
        },
