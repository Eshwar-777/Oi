...
import {
  applyRuntimeManifestInstallCommonFields,
  getFrontmatterString,
  normalizeStringList,
  parseRuntimeManifestInstallBase,
  parseFrontmatterBool,
  resolveRuntimeManifestBlock,
  resolveRuntimeManifestInstall,
  resolveRuntimeManifestOs,
  resolveRuntimeManifestRequires,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseRuntimeManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
  const spec = applyRuntimeManifestInstallCommonFields<SkillInstallSpec>(
    {
...
): RuntimeSkillMetadata | undefined {
  const metadataObj = resolveRuntimeManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveRuntimeManifestRequires(metadataObj);
  const install = resolveRuntimeManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveRuntimeManifestOs(metadataObj);
  return {
