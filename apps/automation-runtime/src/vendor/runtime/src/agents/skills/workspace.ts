...
function resolveSkillsLimits(config?: RuntimeConfig): ResolvedSkillsLimits {
...
function loadSkillEntries(
  ...
  const skillEntries: SkillEntry[] = Array.from(merged.values()).map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveRuntimeMetadata(frontmatter),
...
function applySkillsPromptLimits(params: { skills: Skill[]; config?: RuntimeConfig }): {
...
function resolveWorkspaceSkillPromptState(
  ...
  const truncationNote = truncated
    ? `⚠️ Skills truncated: included ${skillsForPrompt.length} of ${resolvedSkills.length}. Run \`runtime skills check\` to audit.`
    : "";
  const prompt = [
    remoteNote,
    truncationNote,
