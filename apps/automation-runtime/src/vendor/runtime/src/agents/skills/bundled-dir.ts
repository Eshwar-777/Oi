...
import { resolveRuntimePackageRootSync } from "../../infra/runtime-root.js";
...
export function resolveBundledSkillsDir(
  ...
  const override = process.env.RUNTIME_BUNDLED_SKILLS_DIR?.trim();
  if (override) {
    return override;
  }
  ...
  try {
    ...
    const packageRoot = resolveRuntimePackageRootSync({
      argv1,
      moduleUrl,
      cwd,
    });
