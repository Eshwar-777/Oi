...
function isWhatsAppConfigured(cfg: RuntimeConfig): boolean {
...
function isGenericChannelConfigured(cfg: RuntimeConfig, channelId: string): boolean {
  const entry = resolveChannelConfig(cfg, channelId);
  return recordHasKeys(entry);
}
...
function collectModelRefs(cfg: RuntimeConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: RuntimeConfig, providerId: string): boolean {
...
function collectCandidateChannelIds(cfg: RuntimeConfig): string[] {
...
function isPluginExplicitlyDisabled(cfg: RuntimeConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: RuntimeConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: RuntimeConfig, pluginId: string): RuntimeConfig {
  const builtInChannelId = normalizeChatChannelId(pluginId);
  if (builtInChannelId) {
    const channels = cfg.channels as Record<string, unknown> | undefined;
