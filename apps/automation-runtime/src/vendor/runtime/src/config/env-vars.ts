...
function collectConfigEnvVarsByTarget(cfg?: RuntimeConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: RuntimeConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

export function collectConfigServiceEnvVars(cfg?: RuntimeConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** @deprecated Use `collectConfigRuntimeEnvVars` or `collectConfigServiceEnvVars`. */
export function collectConfigEnvVars(cfg?: RuntimeConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.RUNTIME_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
