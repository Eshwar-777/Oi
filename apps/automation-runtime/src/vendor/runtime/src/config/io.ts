...
import type { RuntimeConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  runtime config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  runtime config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
function unsetPathForWrite(
  root: RuntimeConfig,
  pathSegments: string[],
): { changed: boolean; next: RuntimeConfig } {
...
function stampConfigVersion(cfg: RuntimeConfig): RuntimeConfig {
...
function warnIfConfigFromFuture(cfg: RuntimeConfig, logger: Pick<typeof console, "warn">): void {
...
export function createConfigIO(overrides: ConfigIoDeps = {}) {
  ...
  function loadConfig(): RuntimeConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(resolvedConfig as RuntimeConfig, {
        env: deps.env,
  ...
  async function writeConfigFile(cfg: RuntimeConfig, options: ConfigWriteOptions = {}) {
    clearConfigCache();
    ...
    const outputConfigBase =
      envRefMap && changedPaths
        ? (restoreEnvRefsFromMap(cfgToWrite, "", envRefMap, changedPaths) as RuntimeConfig)
        : cfgToWrite;
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.RUNTIME_TEST_CONFIG_OVERWRITE_LOG === "1";
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.RUNTIME_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
    ...
    const auditRecordBase = {
      ...
      watchMode: deps.env.RUNTIME_WATCH_MODE === "1",
      watchSession:
        typeof deps.env.RUNTIME_WATCH_SESSION === "string" &&
        deps.env.RUNTIME_WATCH_SESSION.trim().length > 0
          ? deps.env.RUNTIME_WATCH_SESSION.trim()
          : null,
      watchCommand:
        typeof deps.env.RUNTIME_WATCH_COMMAND === "string" &&
        deps.env.RUNTIME_WATCH_COMMAND.trim().length > 0
          ? deps.env.RUNTIME_WATCH_COMMAND.trim()
          : null,
      existsBefore: snapshot.exists,
...
// module scope. `RUNTIME_CONFIG_PATH` (and friends) are expected to work even
...
let runtimeConfigSourceSnapshot: RuntimeConfig | null = null;
let runtimeConfigSnapshotRefreshHandler: RuntimeConfigSnapshotRefreshHandler | null = null;

function resolveConfigCacheMs(env: NodeJS.ProcessEnv): number {
  const raw = env.RUNTIME_CONFIG_CACHE_MS?.trim();
  if (raw === "" || raw === "0") {
    return 0;
  }
  if (!raw) {
...
function shouldUseConfigCache(env: NodeJS.ProcessEnv): boolean {
  if (env.RUNTIME_DISABLE_CONFIG_CACHE?.trim()) {
    return false;
  }
...
export function getRuntimeConfigSourceSnapshot(): RuntimeConfig | null {
...
export async function readBestEffortConfig(): Promise<RuntimeConfig> {
...
export async function writeConfigFile(
  ...
  // edits to runtime.json remain visible (no stale snapshot).
}
