...
import type { RuntimeConfig, ConfigValidationIssue } from "./types.js";
import { RuntimeSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: RuntimeConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: RuntimeConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: RuntimeConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = RuntimeSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as RuntimeConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as RuntimeConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  const gatewayTailscaleBindIssues = validateGatewayTailscaleBind(validated.data as RuntimeConfig);
  if (gatewayTailscaleBindIssues.length > 0) {
    return { ok: false, issues: gatewayTailscaleBindIssues };
  }
  return {
    ok: true,
    config: validated.data as RuntimeConfig,
  };
}
