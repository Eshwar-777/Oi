...
export const POSIX_RUNTIME_TMP_DIR = "/tmp/runtime";
const TMP_DIR_ACCESS_MODE = fs.constants.W_OK | fs.constants.X_OK;

type ResolvePreferredRuntimeTmpDirOptions = {
...
export function resolvePreferredRuntimeTmpDir(
  options: ResolvePreferredRuntimeTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const base = tmpdir();
    const suffix = uid === undefined ? "runtime" : `runtime-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[runtime] tightened permissions on temp dir: ${candidatePath}`);
  ...
  const ensureTrustedFallbackDir = (): string => {
    ...
    if (state === "invalid") {
      ...
      throw new Error(`Unsafe fallback Runtime temp dir: ${fallbackPath}`);
    }
  ...
  const existingPreferredState = resolveDirState(POSIX_RUNTIME_TMP_DIR);
  if (existingPreferredState === "available") {
    return POSIX_RUNTIME_TMP_DIR;
  }
  if (existingPreferredState === "invalid") {
    if (tryRepairWritableBits(POSIX_RUNTIME_TMP_DIR)) {
      return POSIX_RUNTIME_TMP_DIR;
    }
  ...
  try {
    ...
    mkdirSync(POSIX_RUNTIME_TMP_DIR, { recursive: true, mode: 0o700 });
    chmodSync(POSIX_RUNTIME_TMP_DIR, 0o700);
    if (
      resolveDirState(POSIX_RUNTIME_TMP_DIR) !== "available" &&
      !tryRepairWritableBits(POSIX_RUNTIME_TMP_DIR)
    ) {
      return ensureTrustedFallbackDir();
    }
    return POSIX_RUNTIME_TMP_DIR;
