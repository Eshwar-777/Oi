...
export function resolveBrowserConfig(cfg?: {
  ...
  return {
    ...
    color: cfg?.color ?? DEFAULT_RUNTIME_BROWSER_COLOR,
    headless: cfg?.headless ?? true,
    noSandbox: cfg?.noSandbox ?? false,
    attachOnly: cfg?.attachOnly ?? true,
    defaultProfile: cfg?.defaultProfile ?? DEFAULT_RUNTIME_BROWSER_PROFILE_NAME,
    profiles:
      cfg?.profiles ??
      {
        [DEFAULT_RUNTIME_BROWSER_PROFILE_NAME]: {
