...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".runtime");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.RUNTIME_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$RUNTIME_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via RUNTIME_STATE_DIR.
