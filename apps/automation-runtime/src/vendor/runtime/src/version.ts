...
const CORE_PACKAGE_NAME = "runtime";
...
export function resolveRuntimeServiceVersion(
  ...
  return (
    firstNonEmpty(
      env["RUNTIME_VERSION"],
      runtimeVersion,
      env["RUNTIME_SERVICE_VERSION"],
      env["npm_package_version"],
    ) ?? fallback
  );
}

// Single source of truth for the current Runtime version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: typeof __RUNTIME_VERSION__ === "string" ? __RUNTIME_VERSION__ : undefined,
  bundledVersion: process.env.RUNTIME_BUNDLED_VERSION,
});
