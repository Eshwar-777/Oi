    ...
    if (selector || frameSelector) {
      const locator = frameSelector
        ...
        : selector
          ...
          : page.locator(":root");
      // _snapshotForAI only supports page-scoped captures. For scoped/frame requests,
      // use the native accessibility snapshot so the browser tool can stay browser-first
      // instead of failing on an unsupported combination.
