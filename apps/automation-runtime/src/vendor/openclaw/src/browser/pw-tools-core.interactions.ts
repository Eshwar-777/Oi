...
function interactionTargetDescription(opts: { ref?: string; selector?: string }): string {
  const selector = String(opts.selector ?? "").trim();
  if (selector) {
    return selector;
  }
  return requireRef(opts.ref);
}
...
export async function clickViaPlaywright(opts: {
  ...
  const page = await getRestoredPageForTarget(opts);
  const locator = interactionLocator(page, opts);
  const target = interactionTargetDescription(opts);
  ...
  try {
    ...
    throw toAIFriendlyError(err, target);
  }
}
