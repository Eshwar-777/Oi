...
export function shouldNotifyInBrowser(
  ...
  if (event.type === "run.completed" || event.type === "schedule.created") {
    return preferences.urgency_mode === "all";
  }
  return false;
}
...
export function getNotificationBody(event: AutomationStreamEvent): string | null {
  ...
  if (event.type === "run.completed") {
    return typeof event.payload?.message === "string" ? event.payload.message : "The automation finished successfully.";
  }
  if (event.type === "run.failed") {
    return typeof event.payload?.message === "string" ? event.payload.message : "The automation failed.";
  }
  if (event.type === "schedule.created") {
    const runTimes = Array.isArray(event.payload?.run_times) ? event.payload.run_times : [];
    return typeof runTimes[0] === "string" && runTimes[0] ? `Scheduled for ${runTimes[0]}.` : "A new automation schedule was created.";
  }
  return null;
}

export function getNotificationTitle(event: AutomationStreamEvent): string {
  if (event.type === "run.completed") return "Automation completed";
  if (event.type === "run.failed") return "Automation failed";
  if (event.type === "schedule.created") return "Schedule created";
